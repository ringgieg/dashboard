...
日志级别字段的 key **必须**为 `level`，其他 key 名称不会被识别。
...
级别过滤采用**阈值模式**：选择某级别时，显示该级别及更严重的所有日志（例如选 WARN 则显示 CRITICAL、ERROR、WARN）。

### logLevels 配置

`logLevels` 数组控制日志级别筛选下拉框中显示的选项，按严重程度从高到低排列。可在 `public/config.js` 的各服务配置中自定义：

```js
logLevels: ['CRITICAL', 'ERROR', 'WARN', 'INFO', 'DEBUG', 'TRACE', 'UNKNOWN']
```

> **注意**：`level` 值区分大小写，日志中的级别必须与上表完全一致（全大写）才能正确着色和过滤。
